...
float CalculateShadow()
{
    ...
    for(int x = -1; x <= 1; ++x)
    {
        ...
        {
            // 计算周围的 pcf depth
            float pcfDepth = texture(U_ShadowMap, fragPos.xy + vec2(x, y) * texelSize).r; 
...
void main()
{
    ...
    vec4 diffuseColor = U_DiffuseLightColor * U_DiffuseMaterial * diffuseIntensity * U_DiffuseIntensity;
    ...
    if (shadow > 0.0)
    {
        color += (diffuseColor*attenuation + specularColor*attenuation) * vec4(vec3(0.0), 1.0);
    }
    else
    {
        color += (diffuseColor*attenuation + specularColor*attenuation);
    }
